...
<body class="font-sans bg-gray-100 min-h-screen overflow-x-hidden">
    ...
    <aside class="fixed top-0 left-0 h-screen w-60 bg-gradient-to-b from-blue-500 to-blue-700 text-white shadow-lg">
        <div class="text-center border-b border-white/30 pb-4 mb-6 px-4 pt-6">
            ...
            <ul class="space-y-1 px-3">
                ...
                <li>
                    {{-- <a href="{{ route('admin.data.index') }}" 
                       class="flex items-center px-4 py-2 rounded-md hover:bg-white/20 transition">
                        <i class="fas fa-database mr-3 w-5"></i> Data
                    </a> --}}
                </li>
            </ul>
        </nav>
    ...
    <main class="ml-60 bg-white min-h-screen">
        <!-- Navbar -->
        <nav class="fixed top-0 left-60 right-0 bg-white border-b border-gray-200 shadow flex justify-between items-center px-6 py-4 z-10">
            <span class="font-bold text-lg text-blue-600">
                <i class="fas fa-cloud-sun mr-2"></i> BMKG Admin
            </span>
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="px-4 py-2 rounded-md bg-red-600 text-white text-sm font-medium shadow hover:bg-red-700 transition">
